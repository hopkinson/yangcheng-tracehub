...
if [ -x "/usr/local/bin/prisma" ]; then
  /usr/local/bin/prisma db push --skip-generate || echo "⚠️ [Init] prisma db push 执行遇告警，继续启动应用..."
elif command -v prisma >/dev/null 2>&1; then
  prisma db push --skip-generate || echo "⚠️ [Init] prisma db push 执行遇告警，继续启动应用..."
elif [ -x "./node_modules/.bin/prisma" ]; then
  ./node_modules/.bin/prisma db push --skip-generate || echo "⚠️ [Init] prisma db push 执行遇告警，继续启动应用..."
else
