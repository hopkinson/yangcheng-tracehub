...
import { ThermometerSnowflake, Plus, Trash2, Edit2, Check, X } from "lucide-react";
...
export function ColdStoreDialog({
  ...
  stores: Array<{ id: string; code: string; name: string; _count?: { logs: number } }>;
}) {
  ...
  const handleStartEdit = (s: { id: string; name: string }) => {
    setEditingId(s.id);
    setEditName(s.name);
  };
  ...
  const handleSaveEdit = (id: string) => {
    ...
    startTransition(async () => {
      const res = await updateColdStoreAction(id, { name: editName.trim() });
      if (res.success) {
        toast.success(res.message);
        setEditingId(null);
      } else {
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            配置预冷保鲜仓位，支持重命名。已有入库台账存量的库位禁止删除。
          </DialogDescription>
        </DialogHeader>
            ...
            return (
              <div key={s.id} className="flex items-center justify-between p-2.5 text-xs">
                <div className="flex items-center gap-1.5">
                  ...
                  {s.name && <span className="font-mono text-[10px] text-muted-foreground">{s.code}</span>}
                  {hasLogs && (
