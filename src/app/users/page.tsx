...
import { ApprovalSettingsCard } from "@/components/users/ApprovalSettingsCard";
import { getApprovalSetting } from "@/lib/approval-settings";
import { APPROVAL_ROLE_LABELS } from "@/config/approval";
...
export default async function UsersPage() {
  ...
  const [users, channels, approvalSetting] = await Promise.all([
  ...
  const approvalUserRoles = new Set([
    "ADMIN",
    approvalSetting.tagClaimRole,
    approvalSetting.outboundRole,
  ]);
  const approvalUserCount = users.filter((user) => approvalUserRoles.has(user.role)).length;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ApprovalSettingsCard setting={approvalSetting} />
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-border/80">
          <CardHeader className="p-3 pb-1.5 flex flex-row items-center justify-between">
            <CardTitle className="text-xs font-medium text-muted-foreground">当前审批角色人员</CardTitle>
            <CheckSquare className="size-3.5 text-amber-600" />
          </CardHeader>
          <CardContent className="p-3 pt-0">
            <div className="text-lg font-bold font-mono">{approvalUserCount} 位</div>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              蟹扣：{APPROVAL_ROLE_LABELS[approvalSetting.tagClaimRole]} · 出库：{APPROVAL_ROLE_LABELS[approvalSetting.outboundRole]}
            </p>
          </CardContent>
        </Card>
